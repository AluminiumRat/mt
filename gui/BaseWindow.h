...
namespace mt
{
  ...
  protected:
    ...
    //  Вызывается перед закрытием окна. Позволяет отменить закрытие.
    //  Если возвращает true, то окно будет закрыто, если возвращает
    //    false, то закрытие окна будет отменено
    virtual bool canClose() noexcept;
