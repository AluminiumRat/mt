...
namespace mt
{
  ...
  //    координата minCorner, то бокс считается невалидным. Валидность влияет
  //    на то, как будут просчитываться взаимодействия с другими объектами.
  //    Например, пересечения и расширение объектов
  struct AABB
  {
    glm::vec3 minCorner;
    glm::vec3 maxCorner;
