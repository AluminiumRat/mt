...
namespace mt
{
  ...
  protected:
    ...
    //  Вызывается при старте нового рабочего буфера. Предназначен для
    //    восстановления биндингов, которые должны быть сквозными на протяжении
    //    работы продюсера (дескриптор сеты и пайплайны)
    //  ВНИМАНИЕ!!! Этот метод может не вызываться для первого буфера.
    virtual void restoreBindings(CommandBuffer& buffer);
