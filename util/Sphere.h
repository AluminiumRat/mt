...
namespace mt
{
  struct Sphere
  {
    inline Sphere(const glm::vec3& theCenter, float theRadius) noexcept;
    ...
    //  Выставить отрицательный радиус
    inline void invalidate() noexcept;

    //  Сместить центр и радиус таким образом, чтобы сфера целиком включала в себя
    //  текущую сферу и other
  ...
  inline void Sphere::translate(const glm::mat4& translationMatrix) noexcept
  {
    ...
    float maxLen2 = glm::dot(xScaleVector, xScaleVector);

    glm::vec3 yScaleVector = glm::abs(translationMatrix[1]);
    maxLen2 = std::max(maxLen2, glm::dot(yScaleVector, yScaleVector));

    glm::vec3 zScaleVector = glm::abs(translationMatrix[2]);
    maxLen2 = std::max(maxLen2, glm::dot(zScaleVector, zScaleVector));

    glm::vec3 xyScaleVector = (xScaleVector + yScaleVector) / 1.4142f;
    maxLen2 = std::max(maxLen2, glm::dot(xyScaleVector, xyScaleVector));

    glm::vec3 xzScaleVector = (xScaleVector + zScaleVector) / 1.4142f;
    maxLen2 = std::max(maxLen2, glm::dot(xzScaleVector, xzScaleVector));

    glm::vec3 yzScaleVector = (yScaleVector + zScaleVector) / 1.4142f;
    maxLen2 = std::max(maxLen2, glm::dot(yzScaleVector, yzScaleVector));

    glm::vec3 xyzScaleVector = (xScaleVector + yScaleVector + zScaleVector) /
                                                                        1.732f;
    maxLen2 = std::max(maxLen2, glm::dot(xyzScaleVector, xyzScaleVector));
